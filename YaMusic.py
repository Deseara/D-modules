...
class YaMusicMod(loader.Module):
    ...
    strings = {
        ...
        "_cfg": {
            "token": "Your Yandex.Music access token",
            "now_playing_text": "Template for now playing message. Available variables: {performer}, {title}, {device}, {volume}, {playing_from}, {link}, {track_id}",
            "autobio_text": "Template for autobio text. Available variables: {performer}, {title}",
            "no_playing_bio_text": "Bio text when nothing is playing",
            "banner_version": "Banner version (old or new)",
        },
    }
    strings_ru = {
        ...
        "_cfg": {
            "token": "Ваш токен доступа Яндекс.Музыки",
            "now_playing_text": "Шаблон сообщения о воспроизведении. Доступные переменные: {performer}, {title}, {device}, {volume}, {playing_from}, {link}, {track_id}",
            "autobio_text": "Шаблон текста автобио. Доступные переменные: {performer}, {title}",
            "no_playing_bio_text": "Текст био, когда ничего не играет",
            "banner_version": "Версия баннера (old или new)",
        },
    }
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                option="quote_media",
                default=False,
                doc="Use InputMediaWebPage for quote effect (auto-upload to hosting)",
                validator=loader.validators.Boolean(),
            ),
            loader.ConfigValue(
                option="invert_media",
                default=False,
                doc="Invert media position (photo on top)",
                validator=loader.validators.Boolean(),
            ),
        )

    async def _upload_to_catbox(self, file_bytes: io.BytesIO) -> str:
    ...
    async def ynowcmd(self, message: telethon.types.Message):
        ...
        # Если включен quote_media - загружаем на хостинг
        media = file
        if self.config["quote_media"]:
            ...
            uploaded_url = await self._upload_to_catbox(file_copy)
            if uploaded_url:
                media = InputMediaWebPage(uploaded_url, optional=True)
